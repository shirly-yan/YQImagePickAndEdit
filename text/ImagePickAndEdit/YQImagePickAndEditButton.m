...
@interface YQImagePickAndEditButton ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,PassImageDelegate>



@end

@implementation YQImagePickAndEditButton

+ (instancetype)buttonWithType:(UIButtonType)buttonType {
 
    
    YQImagePickAndEditButton *b = [[YQImagePickAndEditButton alloc]init];
...
- (instancetype)init {
    self = [super init];
    if (self) {
        [self addTarget:self.delegate action:@selector(buttonAvtion) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}
...
- (void)buttonAvtion {
    ...
    [chooseImageSheet showInView:self.delegate.view];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:@"public.image"]){
        ...
        captureView.ratio = self.frame.size.width/self.frame.size.height;
